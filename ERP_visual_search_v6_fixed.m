...
%
%  无目标复合标记编码（+1000）：
%    1101/1102/1103 = A刺激 + 无目标 + 正确/错误/无反应
%    1201/1202/1203 = B刺激 + 无目标 + 正确/错误/无反应
%    1301/1302/1303 = C刺激 + 无目标 + 正确/错误/无反应
%
%  [重要修复说明 v6.1]:
%    实验程序中，标记12（正确反应）只在有目标(41)试次中产生。
%    对于无目标(42)试次，被试按键产生标记22，不按键产生标记32，
%    不会产生标记12。因此无目标条件的提取使用标记22（有按键反应）
%    或合并所有无目标试次（标记22+32），以确保有足够的 epoch。
%% ========================================================================

clear; clc;
script_version = 'v6.1_2026-03-08_fixed';
fprintf('\n====== ERP分析脚本 版本: %s ======\n', script_version);
fprintf('修复内容: 无目标条件提取（标记12在无目标试次中不存在的问题）\n\n');
eeglab;

%% ========================= 参数设置（需要修改的部分）=========================
file_path = 'D:\实验一数据\闪烁光实验一\预处理结束\'; %%% 数据文件所在路径，修改为你的实际路径
save_figure_dir = 'C:\Users\wangw\Desktop\ERP-视觉搜索阶段-v6\v6_中性电极成分'; %%% 保存输出图片所在路径，修改为你的实际路径
saca_csv_dir = 'C:\Users\wangw\Desktop\ERP-视觉搜索阶段-v6\v6_中性电极成分'; %%% 保存CSV数据输出所在路径（方便后续统计软件分析），修改为你的实际路径
...
%%% =================== 无目标条件提取策略（v6.1 新增）===================
%%% 在本实验中，标记12（正确反应）只在有目标试次中产生。
%%% 无目标试次的反应只有标记22（有按键反应）和标记32（无反应）。
%%% 因此需要选择合适的提取策略：
%%%
%%% 'responded'    — 只使用有按键反应的无目标试次（标记22，编码1102/1202/1302）
%%%                   优点：被试有注意到刺激并做出反应，试次质量较高
%%% 'all'          — 使用所有无目标试次（标记22+32，编码1102+1103/1202+1203/1302+1303）
%%%                   优点：试次数量最多，平均更稳定
nt_strategy = 'all';  %%% 'responded' 或 'all'，推荐 'all' 以获得最大试次数
...
for i = 1:nSubj
    ...
    if ~is_epoched
        ...
        NT_responded_markers = [1102, 1202, 1302];   % 无目标 + 有按键反应(标记22)
        NT_noresp_markers    = [1103, 1203, 1303];   % 无目标 + 无反应(标记32)
        NT_correct_markers_orig = [1101, 1201, 1301]; % 无目标 + 标记12（实验中不产生）
        NT_names_resp   = {'A_NT_responded', 'B_NT_responded', 'C_NT_responded'};
        NT_names_noresp = {'A_NT_noresp', 'B_NT_noresp', 'C_NT_noresp'};
        
        for c = 1:3
            n_resp   = sum(epoch_condition == NT_responded_markers(c));
            n_noresp = sum(epoch_condition == NT_noresp_markers(c));
            n_orig   = sum(epoch_condition == NT_correct_markers_orig(c));
            fprintf('    刺激%d: 有反应(22)=%d, 无反应(32)=%d, 标记12=%d\n', ...
                c, n_resp, n_noresp, n_orig);
        end
        ...
        % ---- [修复] 无目标条件提取 ----
        % 根据 nt_strategy 选择提取策略
        for c = 1:3
            if strcmpi(nt_strategy, 'responded')
                % 只用有按键反应的无目标试次（标记22 → 编码 1102/1202/1302）
                nt_epoch_idx = find(epoch_condition == NT_responded_markers(c));
                nt_label = sprintf('有反应(22), 编码%d', NT_responded_markers(c));
            else
                % 使用所有无目标试次（标记22 + 标记32）
                nt_epoch_idx = find(epoch_condition == NT_responded_markers(c) | ...
                                    epoch_condition == NT_noresp_markers(c));
                nt_label = sprintf('全部(22+32), 编码%d+%d', NT_responded_markers(c), NT_noresp_markers(c));
            end
            
            fprintf('  [无目标] 刺激%d (%s): %d 个 epoch\n', c, nt_label, length(nt_epoch_idx));
            if ~isempty(nt_epoch_idx)
                EEG_temp = pop_select(EEG, 'trial', nt_epoch_idx);
                data_nt(i, c, :, :) = squeeze(mean(EEG_temp.data, 3));
            else
                warning('  被试 %d 无目标条件 %d 无有效 epoch！', i, c);
...
save(save_path, 'data', 'data_nt', 'EEG', 'Subj', 'SubjFiles', 'Cond_names', 'Cond_markers', ...
    'chan_idx', 'chan_of_interest', 'chan_indices', 'chans_of_interest', 'srate', 'nt_strategy');
fprintf('\n数据已保存至: %s\n', save_path);
fprintf('data 维度: %s (被试 × 条件 × 电极 × 时间点)\n', mat2str(size(data)));
fprintf('data_nt 维度: %s (被试 × 条件 × 电极 × 时间点)\n', mat2str(size(data_nt)));
...
sgtitle(sprintf('图%d 各条件ERP地形图时空演变 (每%dms, \\pm%dms平均)', ...
    fig_num, topo_step_ms, topo_half_win), 'fontsize', 13, 'FontWeight', 'bold');

try
    save_fname = fullfile(save_figure_dir, 'Topomap_timeseries_all_conditions.png');
    saveas(gcf, save_fname);
    fprintf('已保存地形图序列至: %s\n', save_fname);
    save_fname_fig = fullfile(save_figure_dir, 'Topomap_timeseries_all_conditions.fig');
    savefig(gcf, save_fname_fig);
    fprintf('已保存 .fig 文件至: %s\n', save_fname_fig);
catch
    warning('保存地形图序列失败，请检查 save_figure_dir 是否可写。');
end
...
N2_search_window = [150 350];
P3_search_window = [250 600];
...
for ci = 1:nChans
    ...
    fname = fullfile(saca_csv_dir, sprintf('N2_mean_amplitude_%s.csv', ch_name));
    fid = fopen(fname, 'w');
    ...
    fname = fullfile(saca_csv_dir, sprintf('P3_mean_amplitude_%s.csv', ch_name));
    fid = fopen(fname, 'w');
    fprintf(fid, csv_header);
    for si = 1:nSubj
        fprintf(fid, '%s,%.4f,%.4f,%.4f\n', SubjNames{si}, ...
    ...
    fname = fullfile(saca_csv_dir, sprintf('N2_peak_latency_%s.csv', ch_name));
    fid = fopen(fname, 'w');
    fprintf(fid, csv_header);
    for si = 1:nSubj
    ...
    fname = fullfile(saca_csv_dir, sprintf('P3_peak_latency_%s.csv', ch_name));
    fid = fopen(fname, 'w');
    fprintf(fid, csv_header);
    for si = 1:nSubj
        fprintf(fid, '%s,%.1f,%.1f,%.1f\n', SubjNames{si}, ...
...
for ci = 1:nChans
    ...
    fname = fullfile(saca_csv_dir, sprintf('TargetERP_N2_amplitude_%s.csv', ch_name));
    fid = fopen(fname, 'w');
    ...
    fname = fullfile(saca_csv_dir, sprintf('TargetERP_P3_amplitude_%s.csv', ch_name));
    fid = fopen(fname, 'w');
    fprintf(fid, 'Subject,A_correct,B_correct,C_correct\n');
    for si = 1:nSubj
...
for i = 1:length(all_figs)
    ...
    try
        ...
        savefig(h, fullfile(save_figure_dir, [fig_name_clean '.fig']));
    catch ME
